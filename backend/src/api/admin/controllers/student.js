const {
  helper_is_job_eligible,
  helper_get_applications,
} = require("../../student/controllers/util");
  ...
  async get_eligible_jobs(ctx) {
    ...
    if (!roll) {
      return ctx.badRequest(null, [
        {
          messages: [
            { id: "Missing roll number", received_roll: roll || null },
          ],
        },
      ]);
    }
    ...
    if (cpi === undefined) {
      return ctx.badRequest(null, [
        { messages: [{ id: "CPI not updated yet" }] },
      ]);
    }
    ...
    const selected_applications = await strapi.db
     ...
     */
    const is_eligible = await Promise.all(
      eligible_jobs.map(
        async (job) =>
          await helper_is_job_eligible(
            student_self,
            job,
  ...
  async resume_zip(ctx) {
    ...
    if (!rolls) {
      return ctx.badRequest(null, [
        {
          messages: [
            { id: "Missing rolls number", received_rolls: rolls || null },
          ],
        },
      ]);
    }
    ...
    await Promise.all(
      roll_arr.map(async (roll) => {
        ...
        if (!student) {
          return ctx.badRequest(null, [
            { messages: [{ id: `Student ${roll} not found` }] },
          ]);
        }

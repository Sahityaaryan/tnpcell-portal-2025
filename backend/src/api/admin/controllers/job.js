  ...
  async get_eligible_students(ctx) {
    ...
    if (!jobId) {
      return ctx.badRequest(null, [
        { messages: [{ id: "Missing job id", received_jobId: jobId || null }] },
      ]);
    }
    ...
    if (approval_status !== "approved") {
      return ctx.badRequest(null, [
        { messages: [{ id: "Job not approved yet" }] },
      ]);
    }

    // console.log(6);

    let eligible_students = await strapi.db
    ...
    if (!Array.isArray(eligible_students)) {
      return ctx.internalServerError(null, [
        { messages: [{ id: "Could not get eligible students" }] },
      ]);
    }

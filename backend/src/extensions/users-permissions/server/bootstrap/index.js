...
const { keys, isEqual, merge } = require('lodash/fp');
...
const initGrant = async (pluginStore) => {
  ...
  if (!prevGrantConfig || !isEqual(prevGrantConfig, grantConfig)) {
    // merge with the previous provider config.
    keys(grantConfig).forEach((key) => {
      if (key in prevGrantConfig) {
        grantConfig[key] = merge(grantConfig[key], prevGrantConfig[key]);
      }
    });
